...
if [ ${apps[0]} == 0 ]; then
        kill $(pgrep yad)
        yad --notification --image="switcher-light" --command="${file2}" --text="Switch to Dark Mode" &
        ...
        kill $(pgrep yad)
        yad --notification --image="switcher-dark" --command="${file2}" --text="Switch to Light Mode" &
